...
Input_shape = (210, 160, 4)  # input image size to model
Action_size = 14
...
def pre_processing(observation):
    ...
    processed_observation = resize(processed_observation, (210, 160), mode='constant')
...
def train():
    ...
    env_to_wrap = gym.make('KungFuMaster-v0')

    env = gym.wrappers.Monitor(env_to_wrap, 'recording_2', force=True)
    ...
    while episode_number < FLAGS.n_episodes:
        ...
        state_history = np.reshape([state_history], (1, 210, 160, 4))
        ...
        while not done:
            ...
            next_state = np.reshape([next_state], (1, 210, 160, 1))
            ...
            if done:
                ...
                if episode_number % 100 == 0 or (episode_number + 1) == FLAGS.n_episodes:
                    file_name = "tennis_model_{}.h5".format(episode_number)
